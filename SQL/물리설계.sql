...
AS
SELECT C.CHECK_NO, S.CSTICKER_NO, S.CONTENT, B.AC_NO
...
AS
SELECT V.DONGNO, S.CSTICKER_NO, S.CONTENT, B.AC_NO
FROM LOCALINFOVIEW V, LOCAL_BOOKMARK B, LOCALTAG T, STICKER S
WHERE V.DONGNO(+) = B.DONG_NO
  AND B.LO_B_NO(+) = T.LO_B_NO
  AND S.CSTICKER_NO(+) = T.CSTICKER_NO;
...
AS 
SELECT C.CHECK_NO, S.CSTICKER_NO, S.CONTENT, C.AC_NO
FROM CHECKLIST C, MYCHECKTAG T, STICKER S
WHERE C.CHECK_NO(+) = T.CHECK_NO
  AND S.CSTICKER_NO(+) = T.CSTICKER_NO;
...
--○ PLUS_TIME, PLUS_SCORE의 TITLE 조건이 잘못 되어있어서 변경 완

ALTER TABLE PLUS_TIME

MODIFY TITLE VARCHAR2(50);

--==>> Table PLUS_TIME이(가) 변경되었습니다.
...
--==>> Table PLUS_SCORE이(가) 변경되었습니다.

 
--○ 미화 : 모든 스티커를 조회하는 뷰
CREATE OR REPLACE VIEW VIEW_ALLSTICKER
AS
SELECT '나의 체크리스트' AS TYPE, CHECK_NO AS NO, CSTICKER_NO, CONTENT, AC_NO
FROM VIEW_MYCHECKSTICKER
UNION ALL
SELECT '북마크 체크리스트' AS TYPE, CHECK_NO AS NO, CSTICKER_NO, CONTENT, AC_NO 
FROM VIEW_CHECKSTICKER
UNION ALL
SELECT '북마크 지역정보' AS TYPE, DONGNO AS NO, CSTICKER_NO, CONTENT, AC_NO
FROM VIEW_LOCALSTICKER;

--○ 모든 체크리스트를 조회하는 뷰 (번호 추가)
CREATE OR REPLACE VIEW VIEW_ALLCHECKLIST
AS 
SELECT '나의 체크리스트' AS TYPE, CHECK_NO AS NO, GUNAME, DONGNAME, TITLE, AC_NO
     , NVL(DEPOSIT, -1) AS DEPOSIT
     , NVL(MWOLSE, -1) AS WOLSE
     , NVL(MJEONSE, -1) AS JEONSE
     , NVL(MMAEMAE, -1) AS MAEMAE
     , NVL(SEC_SCORE, -1) AS SEC
     , NVL(TP_SCORE, -1) AS TP
     , NVL(HJ_SCORE, -1) AS HJ
     , NVL(MART + LAUNDRY+ HOSPITAL + FOOD + CULTURE + PARK, -1) AS CONVENIENCE_TOT
     , NVL(PET_SCORE, -1) AS PET
FROM VIEW_CHECKLIST
UNION ALL
SELECT '북마크 체크리스트' AS TYPE, C.CHECK_NO AS NO, C.GUNAME, C.DONGNAME, C.TITLE, B.AC_NO
     , NVL(C.DEPOSIT, -1) AS DEPOSIT
     , NVL(C.MWOLSE, -1) AS WOLSE
     , NVL(C.MJEONSE, -1) AS JEONSE
     , NVL(C.MMAEMAE, -1) AS MAEMAE
     , NVL(C.SEC_SCORE, -1) AS SEC
     , NVL(C.TP_SCORE, -1) AS TP
     , NVL(C.HJ_SCORE, -1) AS HJ
     , NVL(C.MART + C.LAUNDRY+ C.HOSPITAL + C.FOOD + C.CULTURE + C.PARK, -1) AS CONVENIENCE_TOT
     , NVL(C.PET_SCORE, -1) AS PET
FROM VIEW_CHECKLIST C RIGHT JOIN CHECK_BOOKMARK B
ON C.CHECK_NO = B.CHECK_NO
UNION ALL
SELECT '북마크 지역정보' AS TYPE, L.DONGNO AS NO, L.GUNAME, L.DONGNAME, (L.GUNAME || ' ' || L.DONGNAME || ' 평균') AS TITLE, B.AC_NO AS AC_NO
     , NVL(ROUND(L.WOLSEDEPOSITAVG, 0), -1) AS DEPOSIT
     , NVL(ROUND(L.WOLSEAVG, 0), -1) AS WOLSE
     , NVL(ROUND(L.JEONSEAVG, 0), -1) AS JEONSE 
     , NVL(ROUND(L.MAEMAEAVG, 0), -1) AS MAEMAE
     , NVL(ROUND(L.SECURITYAVG, 1), -1) AS SEC
     , NVL(ROUND(L.TRANSPORTAVG, 1), -1) AS TP
     , NVL(ROUND(L.HONJAPAVG, 1), -1) AS HJ
     , NVL(ROUND(L.CONVENIENCEAVG, 1), -1) CONVENIENCE_TOT
     , NVL(ROUND(L.PETAVG, 1), -1) AS PET
FROM LOCALINFOVIEW L JOIN LOCAL_BOOKMARK B
ON L.DONGNO = B.DONG_NO;
...
AS 
SELECT WI.ID"WITHDRAWAL_ID", WI.TEL"WITHDRAWAL_TEL", WI.NAME"WITHDRAWAL_NAME"
, WI.WIDATE"WITHDRAWAL_DATE", WI.WI_NO"WITHDRAWAL_NO", WIWY.WHY"REASON"
